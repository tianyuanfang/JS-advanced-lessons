...
var map = new Map();
map.set(['a'], 555);
map.get(['a']); // undefined->['a']===['a'] //false

//思考：
var map = new Map();
map.set('a', 555);
map.get('a'); //555  'a'==='a' //true
...
for (let item of map.entries()) { //item是一个数组
...
//Key: 1, Value: one Map(3) {1 => "one", 2 => "two", 3 => "three"}
//Key: 2, Value: two Map(3) {1 => "one", 2 => "two", 3 => "three"}
//Key: 3, Value: three Map(3) {1 => "one", 2 => "two", 3 => "three"}
...
function objToStrMap(obj) {
    let strMap = new Map();
    for (let k of Object.keys(obj)) { //Object.keys(obj)是个数组
        strMap.set(k, obj[k]);
    }
    return strMap;
}
